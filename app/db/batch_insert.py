...
핵심 개념:
1. Chunking: 대량 데이터를 500개씩 나눠서 처리 (DB 연결 풀 제한, 메모리 효율성 고려)
...
def chunked_insert(
    ...
    설계 고려사항:
        - 배치 크기: DB 연결 풀 제한과 메모리 효율성 고려하여 500개로 설정
        - 에러 처리: 일부 배치 실패해도 나머지 계속 처리하여 데이터 손실 최소화
        - 성능: 단건 Insert 대비 약 20배 빠른 처리 속도 달성
    """
    total = len(records)
